    ...
    public static function createUserTable(): mixed
    {
        ...
        if (empty($users)) {
            if (DB_CONFIG['DRIVE'] != "") {
                ...
                if (DB_CONFIG['DRIVE'] == "mysql") $user_table->int('id')->primary()->increment();
                if (DB_CONFIG['DRIVE'] == "pgsql") $user_table->serial('id')->primary();

                $user_table->varchar("username", 50)->notNull();
                $user_table->varchar("password", 150)->notNull();

                if (DB_CONFIG['DRIVE'] == "mysql") $user_table->createdUpdatedAt();
